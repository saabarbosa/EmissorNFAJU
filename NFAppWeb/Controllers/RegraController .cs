...
namespace NFAppWeb.Controllers
{
    ...
    public class RegraController : Controller
    {
        ...
        public JsonResult Get()
        {
            ...
            IList<RegraTO> dados = RegraDAL.Get(start, length, ref totRegistros, textoFiltro, ref totRegistrosFiltro, sortColumn, sortColumnDir);
            if (start > 0 && dados.Count == 0)
            {
                start -= length;
                dados = RegraDAL.Get(start, length, ref totRegistros, textoFiltro, ref totRegistrosFiltro, sortColumn, sortColumnDir);
